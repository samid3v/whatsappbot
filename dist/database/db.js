...
exports.pvpStatsOps = exports.pvpOps = exports.settingsOps = exports.logOps = exports.tournamentOps = exports.statsOps = exports.userOps = void 0;
...
try {
    db.exec("ALTER TABLE pvp_matches ADD COLUMN status TEXT NOT NULL DEFAULT 'pending'");
}
catch { }
try {
    db.exec("ALTER TABLE pvp_matches ADD COLUMN approved_by TEXT");
}
catch { }
try {
    db.exec("ALTER TABLE pvp_matches ADD COLUMN rejection_reason TEXT");
}
catch { }
// ==================== PREPARED STATEMENTS ====================
// User queries
...
function findUserByJid(jid) {
    ...
    const exact = stmts.findUserExact.get(jid);
    if (exact && exact.is_muted === 1)
        return exact;
    if (exact) {
        // Check if there's a muted version with same phone
        const muted = stmts.findUserMutedByPhone.get(phoneNumber);
        if (muted)
            return muted;
        return exact;
    }
    ...
    if (muted)
        return muted;
    const any = stmts.findUserByPhone.get(phoneNumber);
    return any || null;
}
// ==================== USER OPERATIONS ====================
exports.userOps = {
    ...
    incrementMutedMessageCount: (jid) => {
        exports.userOps.getOrCreate(jid);
        stmts.incrementMutedMsgCount.run(jid);
        const row = stmts.getMutedMsgCount.get(jid);
    ...
    hasMutedSpamWarning: (jid) => {
        const row = stmts.getMutedSpamWarning.get(jid);
    ...
    incrementLinkCount: (jid) => {
        ...
        const row = stmts.getLinkCount.get(jid);
...
exports.statsOps = {
    getOrCreate: (userJid) => {
        let stats = stmts.findStats.get(userJid);
        if (!stats) {
...
// ==================== PVP OPERATIONS ====================
exports.pvpOps = {
    recordMatch: (player1Jid, player2Jid, player1Score, player2Score) => {
    ...
    getMatchHistory: (userJid, limit = 10) => {
    ...
    approve: (matchId, adminJid) => {
        stmts.approvePvpMatch.run(adminJid, matchId);
    },
    reject: (matchId, adminJid, reason) => {
        stmts.rejectPvpMatch.run(adminJid, reason, matchId);
    },
    findPending: (player1Jid, player2Jid, player1Score, player2Score) => {
...
exports.pvpStatsOps = {
    getOrCreate: (userJid) => {
        let stats = stmts.findPvpStats.get(userJid);
        if (!stats) {
    ...
    updateAfterMatch: (userJid, goalsFor, goalsAgainst, result, points) => {
    ...
    applyMatchStats: (player1Jid, player2Jid, player1Score, player2Score) => {
        exports.pvpStatsOps.getOrCreate(player1Jid);
        exports.pvpStatsOps.getOrCreate(player2Jid);
        let result1;
        let result2;
        let points1;
        let points2;
        ...
        exports.pvpStatsOps.updateAfterMatch(player1Jid, player1Score, player2Score, result1, points1);
        exports.pvpStatsOps.updateAfterMatch(player2Jid, player2Score, player1Score, result2, points2);
    }
};
